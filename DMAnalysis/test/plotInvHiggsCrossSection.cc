...
void makeBetterLimitPlot(string folder)
{
	...
	//xPts[i] /= 100.;
        ...
        if ( myfile.IsOpen() && !myfile.IsZombie() ) {
            ...
            // Expected  2.5%
            t->GetEntry(0);
            m2s[i] = lim * xSect[i] * bFrac; 
            if( m2s[i]<miny ) miny = m2s[i];

            // Expected  16.0%
            t->GetEntry(1);
            m1s[i] = lim * xSect[i] * bFrac;

            // Expected 50.0%
            t->GetEntry(2);
            exp[i] = lim * xSect[i] * bFrac;

            // Expected 84.0%
            t->GetEntry(3);
            p1s[i] = lim * xSect[i] * bFrac;

            // Expected 97.5%
            t->GetEntry(4);
            p2s[i] = lim * xSect[i] * bFrac;
            if( p2s[i]>maxy ) maxy = p2s[i];

            // Observed limit
            t->GetEntry(5);
            obs[i] = lim * xSect[i] * bFrac;
            obserr[i] = limerr * xSect[i] * bFrac;
...
//
    ...
    */
	...
	grThXs_r = new TGraph(pospts, xPts_r, thxs_r); 
	grThXs_r->SetLineWidth(2); 
	grThXs_r->SetLineStyle(7); 
	grThXs_r->SetLineColor(kRed); 
    }
    ...
    if( savePlots ) {
        ...
        canv->SaveAs( (folder+"/"+plotName+"_sigma.png").c_str() );
        //canv->SaveAs( (folder+"/"+plotName+"_sigma.pdf").c_str() );
        //canv->SaveAs( (folder+"/"+plotName+"_sigma.eps").c_str() );
        canv->SaveAs( (folder+"/"+plotName+"_sigma.root").c_str() );
